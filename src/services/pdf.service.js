...
/**
 * Generate a high-security Identity Forensic Report
 * @param {object} data - { market, stats, anomalies, registrationLogs, generatedBy, signature }
 */
exports.generateIdentityAuditPDF = async (data) => {
    return new Promise((resolve, reject) => {
        const { market, stats, anomalies = [], registrationLogs = [], generatedBy, signature } = data;
        const doc = new PDFDocument({ margin: 40, size: 'A4' });
        const chunks = [];
        ...
        const refNo = `FORENSIC-${market?.uniqueCode || 'MMIS'}-${signature.slice(0, 8)}`;
        drawHeader(doc, 'IDENTITY FORENSIC REPORT', market?.name || 'GLOBAL REGISTRY');

        // Security Stamp
        doc.rect(doc.page.width - 200, 110, 160, 45).fill('#FEE2E2');
        doc.rect(doc.page.width - 200, 110, 160, 45).strokeColor(BRAND.accentRed).lineWidth(1).stroke();
        doc.font('Helvetica-Bold').fontSize(8).fillColor(BRAND.accentRed).text('SECURITY VERIFIED', doc.page.width - 190, 120);
        doc.font('Courier').fontSize(6).fillColor(BRAND.textDark).text(signature, doc.page.width - 190, 134, { width: 140 });

        sectionTitle(doc, 'EXECUTIVE SUMMARY');
        infoRow(doc, 'Registry Scope', market?.name || 'GLOBAL');
        infoRow(doc, 'Total Identities', String(stats?.totalProfiles || 0));
        infoRow(doc, 'Anomaly Alerts', String(stats?.anomalyCount || 0));
        infoRow(doc, 'Generated By', generatedBy || 'System Protocol');

        sectionTitle(doc, 'ANOMALY DETECTION LEDGER (CRITICAL)');
        if (anomalies.length === 0) {
            doc.font('Helvetica').fontSize(10).fillColor(BRAND.accentGreen).text('No security anomalies detected in this reporting period.');
        } else {
            anomalies.slice(0, 15).forEach((a, i) => {
                doc.font('Helvetica-Bold').fontSize(9).fillColor(BRAND.accentRed)
                   .text(`[ALERT] COLLISION: ID ${a.mmisId || 'N/A'}`, 40, doc.y);
                doc.font('Helvetica').fontSize(8).fillColor(BRAND.textDark)
                   .text(`Timestamp: ${new Date(a.timestamp).toLocaleString()} | Severity: ${a.severity}`, 40, doc.y + 12);
                doc.moveDown(1.8);
            });
        }

        sectionTitle(doc, 'RECENT ENROLLMENT HITS');
        registrationLogs.slice(0, 10).forEach(r => {
            doc.font('Helvetica-Bold').fontSize(9).fillColor(BRAND.primaryDark).text(r.name, 40, doc.y);
            doc.font('Helvetica').fontSize(8).fillColor(BRAND.midGray).text(`${r.email} | Registered: ${new Date(r.date).toLocaleDateString()}`, 40, doc.y + 10);
            doc.moveDown(1.8);
        });
